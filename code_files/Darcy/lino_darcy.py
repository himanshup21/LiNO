...
width  = 64
batch_size = 64
data_path  = '/home/phimanshu/SSD/Shared/SP_HP_shared/Data/Darcy/darcy_data_256_256.npz'
S = 2
...
a = torch.tensor(data["w"][:, ::sub, ::sub],      dtype=torch.float32).unsqueeze(1)
u = torch.tensor(data["output"][:, ::sub, ::sub], dtype=torch.float32).unsqueeze(1)
...
class LiftingBlock(nn.Module):
    def __init__(self, channels):
        ...
        in_ch = channels + 2
        self.C = channels
        self.P = LocalMLP(in_ch, 3*channels)
    ...
    def forward(self, x):
        ...
        p_x00_c = self.P(x00_c)
        d01 = x01 - p_x00_c[:,:self.C]
        d10 = x10 - p_x00_c[:,self.C:2*self.C]
        d11 = x11 - p_x00_c[:,2*self.C:3*self.C]
    ...
    def inverse(self, s, d_tuple):
        ...
        x00_c = add_coords(x00)
        p_x00_c = self.P(x00_c)
        x01 = d01 + p_x00_c[:,:self.C]
        x10 = d10 + p_x00_c[:,self.C:2*self.C]
        x11 = d11 + p_x00_c[:,2*self.C:3*self.C]
...
log.info('[2/4] Building LNO \u2026')

model = LNO(in_channels=1, width=width, levels=levels).to(device)
