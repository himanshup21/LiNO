...
width  = 64
batch_size = 128
data_path  = '/home/phimanshu/SSD/Shared/SP_HP_shared/Data/Allen_cahn/Allen_cahn_2d_128_128.mat'
S = 1
...
# ── Data ───────────────────────────────────────────────────────────────────
logger.info(f'[1/4] Loading data from {data_path}')
data = loadmat(data_path)

a = torch.tensor(data['coeff'][:,1::S,1::S],      dtype=torch.float32).unsqueeze(1)
u = torch.tensor(data['sol'][:,1::S,1::S], dtype=torch.float32).unsqueeze(1)
...
logger.info(f'   a shape : {tuple(a.shape)}   u shape : {tuple(u.shape)}')
...
def train():
    ...
    logger.info(f'[4/4] Training complete. Best test rel-L2 = {best_test_rel_l2:.4e}')
    logger.info(f'      Best checkpoint  \u2192 {best_ckpt_path}')
    logger.info(f'      Peak GPU memory usage : {peak_mem_mb:.2f} GB')
