...
data_path = "/home/phimanshu/SSD/Shared/SP_HP_shared/Data/GrayScott/gray_scott_spirals_128.hdf5"
data = h5py.File(data_path, "r")
print("Keys:", list(data.keys()))

U  = torch.tensor(data['A'][:,:T_out+T_in,::sub,::sub], dtype=torch.float32)
V  = torch.tensor(data['B'][:,:T_out+T_in,::sub,::sub], dtype=torch.float32)

# Stack -> (N, 2, T, X, Y)
u_all  = torch.stack([U, V], dim=1)
N, C, T_total, X, Y = u_all.shape

# Per-channel normalisation — shape (1, 2, 1, 1, 1)
u_mean = u_all.mean(dim=(0, 2, 3, 4), keepdim=True)
u_std  = u_all.std( dim=(0, 2, 3, 4), keepdim=True)
u_norm = (u_all - u_mean) / (u_std + 1e-8)   # (N, 2, T_total, X, Y)

u_train = u_norm[:ntrain]
u_test  = u_norm[ntrain:ntrain+ntest]

train_loader = DataLoader(TensorDataset(u_train), batch_size=16, shuffle=True)
...
model = LiNO(in_channels=N_FIELDS * T_in, out_channels=N_FIELDS, width=32, levels=levels).to(device)
...
def model_eval(epoch, loss, t0=t0, inference=False):
    ...
    U_mean = u_mean[:, 0, :, :, :].to(device)   
    U_std  = u_std[ :, 0, :, :, :].to(device)
    V_mean = u_mean[:, 1, :, :, :].to(device)
    V_std  = u_std[ :, 1, :, :, :].to(device)

    with torch.no_grad():
        for i in range(n_samples):
            u_s  = u_test[i:i+1].to(device)               
            ...
            for t in range(0, T_out, step):
                im = model(xx_r)                           
                preds.append(im.unsqueeze(2))              
                xx_r = torch.cat([xx_r[:, N_FIELDS:], im], dim=1)

            pred_seq = torch.cat(preds, dim=2)             

            pred_U = pred_seq[:, 0] * U_std + U_mean   
            true_U = yy_t[   :, 0] * U_std + U_mean
            pred_V = pred_seq[:, 1] * V_std + V_mean  
            true_V = yy_t[   :, 1] * V_std + V_mean

            rel_l2_U = loss_fn(pred_U, true_U)
            rel_l2_V = loss_fn(pred_V, true_V)
            total_rel_l2_U += rel_l2_U.item()
            total_rel_l2_V += rel_l2_V.item()

            std_u.append(rel_l2_U.item())
            std_v.append(rel_l2_V.item())

    avg_rel_l2_U = total_rel_l2_U / n_samples
    avg_rel_l2_V = total_rel_l2_V / n_samples

    avg_std_u = torch.tensor(std_u).std().item()
    avg_std_v = torch.tensor(std_v).std().item()
    t1 = time.time()
    elapsed = (t1 - t0) / 60  

    print(f"Epoch {epoch:4d}  Loss: {loss:.6f}  "
        f"U RelL2: {avg_rel_l2_U:.6f}  V RelL2: {avg_rel_l2_V:.6f}  "
        f"U Std: {avg_std_u:.6f}  V Std: {avg_std_v:.6f}", f"Time: {elapsed:.2f}min")

    # torch.save(model.state_dict(), "lino_GS_2d_ar_best.pth")
    if inference == False:
        torch.save({
                    'epoch':          epoch,
                    ...
                    'test_rel_l2_u':    avg_rel_l2_U,
                    'test_rel_l2_v':    avg_rel_l2_V,
                    'test_std_u':       avg_std_u,
                    'test_std_v':       avg_std_v,
                    'loss':           loss
                }, best_ckpt_path)
...
def visualization():
    ...
    u_field_mean = u_mean[:, FIELD_IDX, :, :, :].to(device)
    u_field_std  = u_std[ :, FIELD_IDX, :, :, :].to(device)
    ...
    with torch.no_grad():
        for row, idx in enumerate(sample_idx):
            ...
            pred_u = (pred_last[:, FIELD_IDX] * u_field_std + u_field_mean).cpu().squeeze().numpy()
            true_u = (yy_t[:, FIELD_IDX, -1]  * u_field_std + u_field_mean).cpu().squeeze().numpy()
            err = np.abs(pred_u - true_u)

            im0 = axes[row, 0].imshow(true_u, cmap='viridis')
            axes[row, 0].set_title(f'U Ground Truth (idx={idx}, t={T_in+T_out-1})', fontsize=TITLE_FONTSIZE)
            cbar0 = plt.colorbar(im0, ax=axes[row, 0], fraction=0.046)
            cbar0.ax.tick_params(labelsize=CBAR_FONTSIZE)

            im1 = axes[row, 1].imshow(pred_u, cmap='viridis')
            axes[row, 1].set_title(f'U Prediction (idx={idx}, t={T_in+T_out-1})', fontsize=TITLE_FONTSIZE)
            ...
            axes[row, 2].set_title(f'U Absolute Error (idx={idx})', fontsize=TITLE_FONTSIZE)
